...
public class ShoppingCart {
    ...
    public String addToCart(String foodName, String restaurantId, boolean isPartyFood, int userId) throws SQLException, Error403 {
        int foodIndex = contain(foodName, isPartyFood);
        if(foodIndex == -1){
            if(!isPartyFood)
                UserRepository.getInstance().insertInCart(foodName, restaurantId, userId);
            else {
                UserRepository.getInstance().insertPartyFoodInCart(foodName, restaurantId, userId);
            }
        }
        else{
            if(!isPartyFood)
                UserRepository.getInstance().updateInCart(foodName,userId);
            else
                if(isPartyFinished())
                    UserRepository.getInstance().updatePartyFoodInCart(foodName, restaurantId, userId);
                else{
                    UserRepository.getInstance().clearCart(userId);
    ...
    public String deleteFromCart(String restaurantId, String foodName, boolean isPartyFood, int userId) throws Error404, SQLException, Error403 {
        int foodIndex = contain(foodName , isPartyFood);
        if(foodIndex == -1){
            ...
            if (!isPartyFood) {
                if (UserRepository.getInstance().deleteFromCart(foodName, userId, items.get(foodIndex).getNumber() == 1, items.get(foodIndex).getFood().getPrice())) {
                    UserRepository.getInstance().clearCart(userId);
                }
            }
            else {
                if (isPartyFinished()) {
                    if (UserRepository.getInstance().deletePartyFoodFromCart(restaurantId, foodName, userId, items.get(foodIndex).getNumber() == 1, items.get(foodIndex).getFood().getPrice())) {
                        UserRepository.getInstance().clearCart(userId);
                    }
                }
                else {
    ...
    public int finalizeOrder(int userCredit, int orderId, int userId) throws Error403, Error400, SQLException {
        if(isEmpty){
        ...
        if (userCredit >= totalPayment){
            UserRepository.getInstance().finalizeOrder(userId, orderId);
            UserRepository.getInstance().clearCart(userId);
            return orderId;
        }
        else
